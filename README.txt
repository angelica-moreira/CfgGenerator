...
To generate a Makefile, execute the build.sh script which is inside the folder that contains the source code of this
first project, which is TP1/CfgGenerator/src, indicating the root path where your LLVM is built, as follows:

./build.sh path/to/llvm # you should run this command inside the folder src     

This script will execute the following command: cmake -DBUILD_SHARED_LIBS=ON $LLVM_BUILD_DIR ..
...
being executed in our build.sh script as follows: make -j8                     
...
do not you can convert it by executing the following command: dot -Tpdf [file_name].dot -o [file_name].pdf.
...
are inside the folder TP1/CfgGenerator/src/TestFiles/stanford                  
